...
document.addEventListener('DOMContentLoaded', function() {
        ...
        restaurants.forEach(function(restaurant) {
            restaurantList.innerHTML += 
                `<div class="restaurant">
                    <p><strong>Restaurant ${restaurant.id}:</strong> ${restaurant.name}</h3>
    ...
    filterButton.addEventListener('click', function() {
        ...
        const cheapRestaurants = restaurants.filter(function(restaurant) {
            return restaurant.priceRange === "$" || restaurant.priceRange === "$$"; // I am trying to return a new array cheapRestaurants filtered with price-ranges.
        });

        filteredList.innerHTML = ''; // Clearing up the placeholder text, otherwise, the new content just gets added below this.
        cheapRestaurants.forEach(function(cheapRestaurants) {
            filteredList.innerHTML += 
                `<div class="restaurant">
                    <p><strong>${cheapRestaurants.name}</strong></p>
                    <p>Price Range: ${cheapRestaurants.priceRange}</p>
                </div>`;
});
    ...
    findButton.addEventListener('click', function() {
        ...
        foundItem.innerHTML = '';
            if (bestRestaurant) {
                foundItem.innerHTML +=
                `<div class="restaurant">
                    <h3>${bestRestaurant.name}</h3>
                    <p><strong>Cuisine: </strong>${bestRestaurant.cuisine}</p>
                    <p><strong>Rating: </strong>${bestRestaurant.rating}</p>
                </div>`;
        } 
            else {
                foundItem.innerHTML +=
                "<p>No Restaurants with rating 4.8 exists. </p>";
        }
        });
