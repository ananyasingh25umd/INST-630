...
document.addEventListener('DOMContentLoaded', function() {
    ...
    function showCardView() {
        ...
        const properT = restaurant.properties;

        const cardView = document.createElement('div');
        cardView.classList.add('restaurant-card');
        cardView.innerHTML = `
            <h3 class="card-name">${properT.name}</h3>
            <div class="card-location"> ${properT.address_line_1}, <br> ${properT.city}, ${properT.state}</div>
            <div class="card-status ${getComplianceStatus(restaurant)}">${getComplianceStatus(restaurant)}</div>
            <div class="card-date"> <strong>Last Inspected:</strong> ${formatDate(properT.inspection_date)}</div>
        `;

        cardGrid.appendChild(cardView);
        });
    ...
    function showTableView() {
        ...
        const handClass = properT.proper_hand_washing === 'In Compliance' ? 'pass' : 'fail';
        const foodClass = properT.cooking_time_and_temperature === 'In Compliance' ? 'pass' : 'fail';

        const row = document.createElement('tr');
        row.classList.add('inspection-table');
        row.innerHTML = `
        <td class="table-restaurant-name">${properT.name}</td>
        <td>${properT.city}</td>
        <td>${formatDate(properT.inspection_date)}</td>
        <td class="table-status ${getComplianceStatus(restaurant)}">${getComplianceStatus(restaurant)}</td>
        <td class="compliance-indicator ${handClass}">${getComplianceIndicator(properT.proper_hand_washing)}</td>
        <td class="compliance-indicator ${foodClass}">${getComplianceIndicator(properT.cooking_time_and_temperature)}</td>
    `;
    ...
    function showStatsView() {
        ...
        // Count compliant and non-compliant restaurants
        const compliant = restaurants.filter(function(r) {
            return getComplianceStatus(r) === 'compliant';
        }).length;

        const nonCompliant = restaurants.filter(function(r) {
            return getComplianceStatus(r) === 'non-compliant';
        }).length;

        const complianceRate = ((compliant / total) * 100).toFixed(1);

        // County-wide stats cards
        statsGrid.innerHTML = `
            <div class="stat-card"><span class="stat-label">Total Restaurants</span> <span class="stat-number"><br>${total}</span></div>
            <div class="stat-card"><span class="stat-label">Compliant</span> <span class="stat-number"><br>${compliant}</span></div>
            <div class="stat-card"><span class="stat-label">Non-Compliant</span> <span class="stat-number"><br>${nonCompliant}</span></div>
            <div class="stat-card"><span class="stat-label">Compliance Rate</span> <span class="stat-number"><br>${complianceRate}%</span></div>
        `;
            ...
            if (!cityArr[city]) {
                cityArr[city] = { total: 0, compliant: 0 };
            }

            cityArr[city].total += 1;
            if (status === 'compliant') {
                cityArr[city].compliant += 1;
            }
        });

        Object.entries(cityArr).forEach(function(entry) {
            const city = entry[0];
            const data = entry[1];
            const rate = ((data.compliant / data.total) * 100).toFixed(1);

            const div = document.createElement('div');
            div.classList.add('city-stat');
            div.innerHTML = `<span class="city-name">${city}</span> <span class="city-compliance">${rate}% compliant (${data.total} restaurants)</span>`;
            cityStats.appendChild(div);
        });
    ...
    function showDataSummary() {
        ...
        const complianceRateDisplay = document.querySelector('#compliance-rate');
        ...
        complianceRateDisplay.textContent = `${complianceRate}%`;
